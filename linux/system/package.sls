...
{%- for name, package in system.package.items() %}
  ...
  {%- if package.repo is defined or package.hold is defined or package.verify is defined or package.source_url is defined %}
linux_extra_package_{{ name }}:
  ...
  - fromrepo: {{ package.repo }}
    {%- endif %}
    {%- if package.source_url is defined %}
  - enable: True
  - sources: 
    - {{ name }}: {{ package.source_url }}
    {%- endif %}
